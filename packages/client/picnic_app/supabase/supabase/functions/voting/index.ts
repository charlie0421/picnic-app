...
async function getUserProfiles(supabaseClient, user_id) {
  try {
    ...
    const { data: user_profiles, error } = await supabaseClient.from('user_profiles').select('id, star_candy, star_candy_bonus').eq('id', user_id);
    if (error) {
...
async function canVote(user_id, vote_amount, vote_pick_id) {
  try {
    const { rows } = await queryDatabase(`
