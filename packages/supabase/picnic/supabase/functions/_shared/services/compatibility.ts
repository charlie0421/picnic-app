...
export class CompatibilityService {
  ...
  async updateCompleted(compatibilityId: string): Promise<Compatibility> {
    try {
  ...
  async generateNewResults(
    compatibility: Compatibility,
  ): Promise<Compatibility> {
    try {
      const prompt = await this.promptService.getActivePrompt(
    ...
    try {
      ...
      const translatedDetails = {
        style: {
          ...
          couple_style: await translateText(
            style.couple_style || '',
            targetLang,
          ),
        },
