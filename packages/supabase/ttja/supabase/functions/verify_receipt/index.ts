...
console.log(
  ...
  GOOGLE_PRIVATE_KEY.substring(0, 100),
);
console.log(
...
Deno.serve(async (request) => {
  ...
  try {
    ...
    const { receipt, platform, productId, user_id, environment } =
      await request.json();
    console.log(
    ...
    if (data.success) {
      ...
      const transactionId = `${platform}_${Date.now()}_${Math.random()
        .toString(36)
        .substr(2, 9)}`;
      await grantReward(
        user_id,
...
async function grantReward(userId, productId, transactionId) {
  try {
    ...
    console.log(
      `Reward granted for user ${userId}: ${JSON.stringify(
        reward,
