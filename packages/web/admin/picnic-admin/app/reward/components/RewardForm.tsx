...
import { Form, Input, InputNumber, Tabs, Upload, FormProps, message, Row, Col, Card } from 'antd';
...
export default function RewardForm({
  ...
  const [initialValues, setInitialValues] = useState<any>(formProps.initialValues || {});
  ...
  useEffect(() => {
    if (formProps.initialValues) {
      ...
      if (values.size_guide_images && Array.isArray(values.size_guide_images)) {
        values.size_guide_images = values.size_guide_images.map((url, index) => ({
          uid: `-${index + 1}`,
      ...
      defaultLocalizations.forEach(locale => {
        ...
        if (typeof values.size_guide[locale] === 'string') {
          ...
          try {
            // JSON 문자열인 경우 파싱
            if (values.size_guide[locale].trim().startsWith('[') || values.size_guide[locale].trim().startsWith('{')) {
          ...
          // 객체인 경우 문자열로 변환
          values.size_guide[locale] = JSON.stringify(values.size_guide[locale], null, 2);
        }
      });
  ...
  const normFile = (e: any) => {
    ...
    // fileList가 없거나 배열이 아닌 경우 빈 배열 반환
    console.log('normFile: fileList가 배열이 아님, 빈 배열 반환');
    return [];
  };
        ...
        <Card title="기본 정보" style={{ marginBottom: 20 }}>
          ...
          {defaultLocalizations.map(locale => (
            <Form.Item
              ...
              name={['title', locale]}
            >
              <Input placeholder={`${getLanguageLabel(locale)} 제목을 입력하세요`} />
            </Form.Item>
          ))}
        </Card>
          ...
          <Row gutter={[24, 24]}>
            <Col span={12}>
              {defaultLocalizations.map(locale => (
                ...
                >
                  <Input.TextArea rows={4} placeholder={`${getLanguageLabel(locale)} 위치 정보를 입력하세요`} />
                </Form.Item>
              ))}
            </Col>
            <Col span={12}>
            ...
            <Col span={12}>
              {defaultLocalizations.map(locale => (
                      ...
                      validator: (_, value) => {
                        ...
                        if (typeof value === 'string' && (value.trim().startsWith('{') || value.trim().startsWith('['))) {
                          try {
                            ...
                            return Promise.reject('유효하지 않은 JSON 형식입니다.');
                          }
                        }
                        return Promise.resolve();
                      }
                ...
                >
                  <Input.TextArea 
                    rows={8} 
                    placeholder={`${getLanguageLabel(locale)} 사이즈 가이드를 입력하세요. 배열 형식의 JSON 데이터를 입력하세요.
