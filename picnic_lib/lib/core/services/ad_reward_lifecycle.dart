...
class AdRewardLifecycle with WidgetsBindingObserver {
    ...
    if (_auth.isEnabled) {
      ...
      _subscription = _auth.authStateChanges.listen(
        ...
        onError: (Object error, StackTrace stack) {
          logger.w('광고 세션 변경 수신 실패', error: error, stackTrace: stack);
        },
      );
    }
  ...
  void didChangeAppLifecycleState(AppLifecycleState state) {
    ...
    _syncOwner(_auth.currentSession?.user.id);
    unawaited(
      _wallet.refreshOnResume().catchError((Object error, StackTrace stack) {
        logger.w('복귀 잔액 갱신 실패', error: error, stackTrace: stack);
