...
class YouTubeMissionGuidePage extends StatefulWidget {
  ...
  @override
  State<YouTubeMissionGuidePage> createState() =>
      _YouTubeMissionGuidePageState();
}
...
class _YouTubeMissionGuidePageState extends State<YouTubeMissionGuidePage> {
  ...
  Future<void> _pickAndUpload() async {
    ...
    try {
      ...
      final path =
          'proofs/${DateTime.now().millisecondsSinceEpoch}_$_clickId.$ext';
      final up = await widget.service.createSignedUploadUrl(path: path);
      await widget.service.uploadWithSignedUrl(
        signedUrl: up.signedUrl,
        file: file,
      );
      final downloadUrl = await widget.service.createSignedDownloadUrl(
        path: path,
      );

      final result = await widget.service.validateProof(
        clickId: _clickId!,
      ...
      body: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const Text(
              '1) 영상 재생 중 화면 한 번 탭 → 시간/진행바 보이게 캡처\n2) 30초 이상 경과가 보이도록 촬영\n3) 갤러리에서 그 1장을 업로드',
            ),
