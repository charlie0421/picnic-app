...
void main() {
  ...
  testWidgets('pouch header keeps a 48px refresh target inside a tight card', (
    tester,
  ) async {
    // PICNIC-2689: 카드는 타이트하되 새로고침의 탭 영역은 머티리얼 최소
    // 조작 영역(48) 을 유지한다 (Codex 리뷰). 시각적 상단 여백은 헤더 행
    // 안에서 아이콘이 세로 중앙에 오므로 4 + 12 = 16 이다.
    var taps = 0;
    await pumpWidgetAndIgnoreErrors(
      tester,
      buildTestApp(
    ...
    final refresh = find.byKey(const Key('store-point-info-refresh'));
    final refreshRect = tester.getRect(refresh);
    expect(refreshRect.size, const Size(48, 48));
    final card = tester.getRect(find.byType(StorePointInfo));
    // 카드 상단 → 헤더 행 상단: 테두리 2 + 패딩 4.
    expect(refreshRect.top - card.top, 6);
    // 탭 영역 가장자리(모서리 안쪽 2px)에서도 눌린다.
    await tester.tapAt(refreshRect.topLeft + const Offset(2, 2));
    await tester.tapAt(refreshRect.bottomRight - const Offset(2, 2));
    expect(taps, 2);
  });

  for (final locale in const ['fil', 'bn', 'my', 'id', 'th', 'vi', 'ja']) {
    testWidgets(
      'wallet header stays on one line at 320px in $locale (PICNIC-2689)',
      (tester) async {
        // 긴 번역(fil/bn/my 등)도 좁은 카드에서 2단으로 쌓이지 않고 축소된다.
        tester.view.physicalSize = const Size(320, 700);
        tester.view.devicePixelRatio = 1;
        addTearDown(tester.view.reset);
        Widget app(String title) => buildTestApp(
          Padding(
            padding: const EdgeInsets.all(16),
            child: StorePointInfo(
              title: title,
        ...
        await pumpWidgetAndIgnoreErrors(tester, app('pouch'));
        await pumpAndIgnoreErrors(tester);
        final localizations = AppLocalizations.of(
          tester.element(find.byType(StorePointInfo)),
        );
        final title = localizations.label_star_candy_pouch;
        final guide = localizations.expiring_bonus_candy_guide;
        // 해당 로케일의 실제 제목 문구로 다시 그린다.
        await pumpWidgetAndIgnoreErrors(tester, app(title));
        await pumpAndIgnoreErrors(tester);
