...
void main() {
  group('WeChat Login - China Environment Tests', () {
    ...
    group('Performance Tests for China Network', () {
      test('should measure login performance under China network conditions',
          () async {
        ChinaNetworkSimulator.enable();
        ...
        await ChinaNetworkSimulator.simulateChinaMobileNetwork(() async {
          await Future.delayed(
              const Duration(milliseconds: 200)); // Auth request
        });

        await ChinaNetworkSimulator.simulateChinaMobileNetwork(() async {
          await Future.delayed(
              const Duration(milliseconds: 150)); // Token exchange
        });
      ...
      test('should handle concurrent login attempts', () async {
        ...
        for (int i = 0; i < 5; i++) {
          futures
              .add(ChinaNetworkSimulator.simulateChinaMobileNetwork(() async {
    ...
    group('Error Handling in China Environment', () {
      ...
      test('should provide meaningful error messages for China-specific issues',
          () async {
        ChinaNetworkSimulator.enable();
    ...
    group('Integration Tests', () {
      test('should generate comprehensive test report', () {
        ...
        expect(
            testSummary['deviceCharacteristics'], isA<Map<String, dynamic>>());
