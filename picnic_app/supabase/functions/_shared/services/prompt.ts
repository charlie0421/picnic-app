...
export class PromptService {
  ...
  async renderPrompt(prompt, variables) {
    try {
      // 필수 변수 검증
  ...
  async logPromptUsage(log) {
    try {
      const { error } = await this.supabase.from('prompt_usage_logs').insert({
        ...log,
  ...
  async getActivePrompt(name) {
    ...
    // 데이터베이스에서 프롬프트 조회
    const { data, error } = await this.supabase.from('prompts').select('*').eq('name', name).eq('is_active', true).order('version', {
      ascending: false
    }).limit(1).single();
    if (error) {
