 ...
 */ export function createCorsHeaders(origin, options = {}) {
  const allowedOrigins = options.allowedOrigins ?? DEFAULT_CORS_OPTIONS.allowedOrigins ?? [];
  const allowedMethods = options.allowedMethods ?? DEFAULT_CORS_OPTIONS.allowedMethods ?? [];
  const allowedHeaders = options.allowedHeaders ?? DEFAULT_CORS_OPTIONS.allowedHeaders ?? [];
  const maxAge = options.maxAge ?? DEFAULT_CORS_OPTIONS.maxAge ?? 86400;
 ...
 */ export function createOptionsResponse(origin, options) {
