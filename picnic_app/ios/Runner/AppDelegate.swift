                    ...
                    if self.isSDKInitialized {
                        ...
                        self.initPangle(appId: appId) { success, errorMessage in
                            if success {
                                ...
                                result(FlutterError(code: "InitFailed", message: "Pangle SDK 초기화 실패: \(errorMessage)", details: nil))
                            }
                        }
                    } else {
                        result(FlutterError(code: "NotInitialized", message: "Pangle SDK가 초기화되지 않았습니다", details: nil))
                    }
                } else {
                    result(FlutterError(code: "InvalidParams", message: "Placement ID is null", details: nil))
                }
            case "showRewardedAd":
...
extension AppDelegate: PAGRewardedAdDelegate {
    ...
    func loadRewardedAd(placementId: String, result: @escaping FlutterResult) {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.5) {
            // 메인 스레드에서 로드 시작
            PAGRewardedAd.load(withSlotID: placementId, request: request) { [weak self] (rewardedAd: PAGRewardedAd?, error: Error?) in
                // 메인 스레드에서 콜백 처리 보장
                DispatchQueue.main.async {
                    guard let self = self else {
                    ...
                    if let error = error {
                        ...
                        print("오류 상세: \(error)")
                        result(FlutterError(code: "LoadFailed", message: error.localizedDescription, details: nil))
                        return
                    }
                    ...
                    if let rewardedAd = rewardedAd {
                        ...
                        if self.rewardedAd != nil {
                            ...
                            print("광고 객체 저장 실패")
                            result(FlutterError(code: "LoadFailed", message: "광고 객체 저장 실패", details: nil))
                        }
                    } else {
                        print("리워드 광고 로드 실패: 광고 객체가 null임")
                        result(FlutterError(code: "LoadFailed", message: "알 수 없는 오류로 광고 로드 실패", details: nil))
                    }
                }
            }
        }
    ...
    func showRewardedAd(result: @escaping FlutterResult) {
        ...
        guard let rootVC = keyWindow.rootViewController else {
            ...
            result(FlutterError(code: "ShowFailed", message: "rootViewController를 찾을 수 없습니다", details: nil))
            return
        }
    ...
    func rewardedAd(_ rewardedAd: PAGRewardedAd, userDidEarnReward rewardModel: PAGRewardModel) {
        print("사용자가 보상을 받음 - 이름: \(rewardModel.rewardName ?? ""), 수량: \(rewardModel.rewardAmount)")
    }
    
    func rewardedAd(_ rewardedAd: PAGRewardedAd, userEarnRewardFailWithError error: Error) {
        print("사용자 보상 지급 실패: \(error.localizedDescription)")
        print("에러 상세: \(error)")
    }
}
