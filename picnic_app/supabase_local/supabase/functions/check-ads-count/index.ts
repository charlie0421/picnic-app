...
Deno.serve(async (req)=>{
  ...
  const { data, error } = await supabase.from('transaction_admob').select('created_at').eq('user_id', user_id).gte('created_at', dayAgo.toISOString());
  if (error) {
    return new Response(JSON.stringify({
      error: 'Error fetching ad view count'
    }), {
      status: 500,
      headers: {
        "Content-Type": "application/json"
