...
export class TapjoyService extends BaseAdService {
  ...
  async verifySignature(params, signature, platform) {
    try {
      ...
      if (!secret) {
        ...
        if (!platformKey) {
          console.error(`TAPJOY_SECRET_${platform?.toUpperCase()} 환경 변수가 설정되지 않았습니다.`);
          return false;
        }
        secret = platformKey;
      }
  ...
  async verify(params) {
    try {
      ...
      // 3. 중복 트랜잭션 확인
      const isDuplicate = await this.checkExistingTransaction(params.id, 'transaction_tapjoy');
      if (isDuplicate) {
        return {
          isValid: false,
      ...
      // 4. 서명 검증
      const isValid = await this.verifySignature(params, params.verifier, params.platform.toLowerCase());
      if (!isValid) {
        return {
          isValid: false,
          error: '서명이 유효하지 않습니다.'
    ...
    try {
      ...
      try {
        ...
        return {
          ...
          body: {
            success: false,
            error: error instanceof Error ? error.message : '트랜잭션 처리 중 오류가 발생했습니다.'
          }
        };
      }
