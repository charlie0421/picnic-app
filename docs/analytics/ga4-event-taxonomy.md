...
### 언어 값 실측 (2026-08-31, 28일)

대행사 문항 **B-6**(`ja`→`jp` 변환)·**B-7**(지원 언어 초과)의 실측 결과다.

| `language` | 총 사용자 | 이벤트 수 | 1인당 |
|---|---:|---:|---:|
| `(not set)` | 8,257 | 549,382 | 67 |
| `en` | 1,533 | **822,525** | 537 |
| `ko` | 2,770 | 376,674 | 136 |
| `vi` | 665 | 292,435 | 440 |
| **`jp`** | 359 | **212,272** | 591 |
| `zh` | 747 | 168,185 | 225 |
| `id` | 656 | 130,221 | 199 |
| `es` | 160 | 128,401 | 803 |
| `th` | 103 | 81,026 | 787 |
| `en_US` | 2,469 | 11,916 | **4.8** |
| `zh_CN` | 556 | 1,757 | **3.2** |
| `zh_TW`·`fil`·`bn`·빈값 | 36 | 103 | — |
| **합계** | 9,359 | 2,774,897 | |

**B-6 통과.** `jp` 가 212,272건 잡히고 **`ja` 는 15개 값 어디에도 없다.**
[Ga4Language.normalize] 가 정상 동작한다.

**B-7 확인.** 스펙은 `ko`/`en`/`jp` 3개인데 실질 세그먼트는 8개다
(`en`·`ko`·`vi`·`jp`·`zh`·`id`·`es`·`th`).

### `(not set)` 은 결함이 아니다 — 초기화 타이밍이다

**사용자 수로는 88%(8,257/9,359)지만 이벤트 수로는 19.8%(549,382/2,774,897)다.**
GA4 사용자 범위 속성은 값이 바뀌면 여러 행에 걸치므로 사용자 수는 중복 집계된다.

앱이 열리고 `main_initializer` 가 [PicnicAnalytics.setUserProperties] 를 호출하기
전까지 나가는 이벤트(`first_open`·`session_start`·초기 `screen_view`)가 `(not set)`
으로 잡히고, **그 구간을 거의 모든 사용자가 한 번씩 지난다.** 그래서 사용자 수로 보면
커 보이고 이벤트 수로 보면 5분의 1이다. `(not set)` 을 "언어 미상 사용자"로 읽으면 안 된다.

### `en_US` / `zh_CN` 은 분석에서 무시한다

1인당 이벤트가 **4.8건 · 3.2건**이다. 같은 언어의 `en`(537건) · `zh`(225건)과 두 자릿수
차이라, 앱을 켠 직후 짧은 순간에만 찍히고 곧 덮어써지는 값으로 보인다.

원인은 `main_initializer.dart` 가 `Intl.getCurrentLocale()` 을 그대로 보내는 것이다 —
사용자가 고른 앱 언어가 아니라 **기기 로케일**이라, 초기화 단계에 따라 `en_US` 와 `en`
이 갈린다. 스펙 §1 의 정의("현재 선택 언어")와 어긋나지만, 실 이벤트 비중이 0.4% 라
수정 우선순위는 낮다. [Ga4Language] 가 `ja` 만 변환하고 나머지를 원문 통과시키는 것은
의도된 설계다(GA4 히스토리 단절 방지).
...
### DB 대조 결과 — 오탐이 아니라 **누락**이다 (2026-09-01)

Supabase MCP 로 프로덕션(`xtijtefcycoeqludlngc`)을 조회했다. **B-9 의 원래 우려(30초 창이
헐거워 오탐)와 정반대 결과다.**

| 기간 | DB 실제 가입 | GA4 `sign_up` 사용자 | 커버리지 |
|---|---:|---:|---:|
| 8/3~8/30 (28일) | 2,747 | 2,026 | 73.8% |
| 8/13~8/30 (택소노미 배포 후) | 2,252 | — | — |
| **8/16~8/29 (깨끗한 창)** | **1,368** | **1,208** | **88.3%** |

8/3~8/12 는 택소노미 배포(1.3.0+130007, 8/13) 전이라 구버전 앱이 `sign_up` 을 보내지
않는다. 그 479건을 빼면 커버리지가 73.8% → 88.3% 로 올라간다.

### 남는 12% 의 원인 — `last_sign_in_at` 은 **마지막** 로그인 시각이다

8/16~8/29 신규 가입 1,418명의 `last_sign_in_at - created_at` 분포:

| | 명 | 비율 |
|---|---:|---:|
| ≤ 30초 (판정 성공) | 1,090 | 76.9% |
| **> 30초 (판정 불가)** | **328** | **23.1%** |
| 최대 격차 | | **1,279,026초 ≈ 14.8일** |

`last_sign_in_at` 은 재로그인할 때마다 갱신된다. **가입 시점에 이벤트를 보내지 못하면**
(오프라인·앱 강제 종료·outbox 지연) 다음 실행 때는 이미 격차가 벌어져 **영구히 신규로
판정되지 않는다.** 최대 격차 14.8일이 그 증거다.

**30초를 늘려도 해결되지 않는다.** 격차가 14.8일까지 벌어지므로 어떤 창도 충분하지 않고,
넓히면 이번에는 진짜 오탐(가입 한참 뒤의 재로그인을 신규로 오분류)이 생긴다.
[AuthAnalyticsResolver.signUpWindow] 를 건드리는 것은 답이 아니다.

**설계 한계이지 결함이 아니다.** Supabase 가 신규 가입 여부를 응답에 주지 않는 이상
이 휴리스틱의 상한이 약 88% 다. 대행사는 §4 표 7번에서 휴리스틱 사용을 이미 허용했으므로,
**`sign_up` 수를 실제 가입 수로 읽지 말 것**만 지키면 된다. 정확한 가입 수가 필요하면
`auth.users` 를 직접 센다.
