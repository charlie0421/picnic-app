...
- 관리자 페이지의 클라이언트 권한 검사와 DB 권한 검사를 모두 유지한다. DB의 `public.is_super_admin()` helper는 이름과 달리 `is_super_admin OR is_admin`을 의미하므로, 충전 내역 RPC의 관리자 범위도 이 helper가 정의하는 관리자다.
